...
I = np.array([[np.exp(-(L-5)**2-(M-5)**2)],
                    [np.exp(-((L-5)**2+(M-5)**2)/8.)]])
N_freq = I.shape[0]
# N_im = I[0].shape[0]*I[0].shape[1] + I[1].shape[0]*I[1].shape[1]

# Construct position vectors for N_im pixels
# I_vec = np.reshape(I, (N_im,), order='F')
I_vec = np.append(np.reshape(I[0], (N_im,), order='F'),
                            np.reshape(I[1], (N_im,), order='F'))
