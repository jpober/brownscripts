...
for chan in $chans; do
    ...
    for pol in $pols; do
		...
		#evenLSTS2=(${EVEN_DATAPATH}${sep}/lst.*.*.*.${SUFFIX2})
                EVEN_FILES="${evenLSTS[@]:9:10}" #${EVEN_DATAPATH}${sep}/ #lst.*.[345]*.*.${SUFFIX} #uvHBFAL was 3:15
                ODD_FILES="${oddLSTS[@]:9:10}" #${ODD_DATAPATH}${sep}/ #lst.*.[345]*.*.${SUFFIX} #uvHBFAL
		...
		echo $EVEN_FILES
                ...
                python ${SCRIPTSDIR}/pspec_cov_v002.py -C ${cal} \
                     -b ${NBOOT} -a ${ANTS} -c ${chan} -p ${pol}\
                      --window=${WINDOW}  ${NOPROJ} --output=${sepdir} \
                       ${EVEN_FILES} ${ODD_FILES} 
