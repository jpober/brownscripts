...
o.add_option('--iter', dest='iter', type='int', default=100,
    help='Deconvolve delay-domain data by the response that results from flagged data.  Specify a tolerance for termination.  Default 1e-5')
o.add_option('--noflag', dest='noflag',default=False, action='store_true',
    help='Deconvolve delay-domain data by the response that results from flagged data.  Specify a tolerance for termination.  Default 1e-5')
...
def getFilters(args,uv):
    v = a.miriad.UV(args[0])
    aa = a.cal.get_aa(opts.cal, uv['sdf'], uv['sfreq'], uv['nchan'])
    filters = C.dspec.wedge_width_by_bl(aa, uv['sdf'], uv['nchan'], offset=opts.offset)
    return filters
...
for uvfile in args:
    ...
    def mfunc(uv, p, d, f):
        ...
        idx1 = Fzero(f,1)
        idx2 = Fzero(f,-1)
        idx2 = n.abs(idx2+uv['nchan']+1)
        #print idx1,idx2
        nchans = idx2-idx1
        window = a.dsp.gen_window(nchans, window=opts.window)
...
#        else:
        ...
        if conj: d = n.conj(d)
        d *= w
        d[idx1:idx2] = d[idx1:idx2]*window
        w[idx1:idx2] = w[idx1:idx2]*window
        _d = n.fft.ifft(d)
        _w = n.fft.ifft(w)
        uthresh,lthresh = filters[(i,j)]
        area = n.ones(_d.size, dtype=n.int)
        area[uthresh:lthresh] = 0
        _d_cl, info = a.deconv.clean(_d, _w, tol=opts.clean, area=area, stop_if_div=False, maxiter=opts.iter)
        if opts.model:
            ...
            d_res = d - d_mdl * w
            d_res[idx1:idx2] = d_res[idx1:idx2]/window
            return p, d_res, f
