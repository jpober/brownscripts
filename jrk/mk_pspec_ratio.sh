...
for chan in $chans; do
    ...
    for pol in $pols; do
		...
		evenLSTS2=(${EVEN_DATAPATH2}${sep}/lst.*.*.*.${SUFFIX2})
                EVEN_FILES="${evenLSTS[@]:12:10}" #${EVEN_DATAPATH}${sep}/ #lst.*.[345]*.*.${SUFFIX} #uvHBFAL was 9:12
                ODD_FILES="${oddLSTS[@]:12:10}" #${ODD_DATAPATH}${sep}/ #lst.*.[345]*.*.${SUFFIX} #uvHBFAL
		EVEN_FILES2="${evenLSTS2[@]:12:10}"
		ODD_FILES2="${oddLSTS2[@]:12:10}"
		echo $EVEN_FILES
